...
sub_model = keras.models.load_model('./model_bottom')
top_model = keras.models.load_model('./model_top')
bottom_model = keras.models.load_model('./model_bottom')
foot_model = keras.models.load_model('./model_shoes')
...
def single_classification(single_path):
    ...
    """
    This function takes a single path of a photo, then does reshape to fit the models, and does classification
    Input is a path of a certain photo
    Output is a tuple which contains subtype (to be sent to a correct sub-model),
    ...
    print(f"Input array shape: {train_images.shape}")  # Print input array shape
    print(f"Output: {res_str}")  # Print output
